...
const TextControl = ({control, entity, count}) => {
  ...
  return (
    <FormControl fullWidth>
      <TextField variant="outlined" required={control.required} name={control.id} label={translate(`control.${label}`)} value={value || control.default || ''} onChange={onChange} error={control.errors && control.errors.length > 0} helperText={helperText}/>
